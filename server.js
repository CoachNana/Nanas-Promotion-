...
const {
  ...
  // Bank details (Config Vars on Heroku)
  BANK_ACCOUNT_NAME,
  BANK_IBAN,
  BANK_BANK_NAME,
  BANK_SWIFT
...
// ---- FRONTEND CONFIG (Stripe publishable key) ----
app.get("/config.js", (_req, res) => {
  res
    .type("application/javascript")
    .send(`window.STRIPE_PUBLISHABLE_KEY=${JSON.stringify(STRIPE_PUBLISHABLE || "")};`);
});
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [{
        price_data: {
          currency: "aed",
          product_data: {
            name: ticketType === "vvip" ? "VVIP Table (8 People)" : `${ticketType} Ticket`
          },
...
app.post("/create-paytabs-session", async (req, res) => {
  try {
    ...
    const payload = {
      ...
      cart_currency: "AED",
      cart_amount: amount,
      cart_description: ticketType === "vvip"
    ...
    saveOrder({
      ...
      gateway: "paytabs",
      status: "PENDING",
      name, phone, ticketType, qty, amount,
      created_at: new Date().toISOString()
    });
...
app.post("/paytabs-callback", express.json(), (req, res) => {
  try {
    ...
    else orders.push({ id: cart_id, gateway: "paytabs", ...update, created_at: new Date().toISOString() });
  ...
  try {
    ...
    const payload = {
      ...
      customer: {
        ...
        phone: { country_code: "971", number: (phone || "").replace(/\D/g, "") || "500000000" }
      },
      description: ticketType === "vvip"
        ? `VVIP Table (8 People) x ${qty || 1}`
        : `${ticketType} ticket(s)`,
    ...
    saveOrder({
      ...
      gateway: "tap",
      status: "PENDING",
      name, phone, ticketType, qty, amount, link,
      created_at: new Date().toISOString()
    });
...
app.post("/tap-callback", express.json(), (req, res) => {
  try {
    const body = req.body || {};
    const ref = (body && body.metadata && body.metadata.ref) || "TAP-" + Date.now();
    const status = body.status || (body.response && body.response.message) || "UNKNOWN";
    ...
    if (idx >= 0) orders[idx] = { ...orders[idx], ...update };
    else orders.push({ id: ref, gateway: "tap", ...update, created_at: new Date().toISOString() });
...
app.get("/admin/orders.csv", (_req, res) => {
  const orders = loadOrders();
  const cols = ["id","gateway","status","name","phone","ticketType","qty","amount","link","gateway_txn","created_at"];
  const header = cols.join(",");
  const rows = orders.map(o => cols.map(k => {
    const v = (o[k] !== undefined && o[k] !== null) ? String(o[k]).replace(/\"/g,'\"\"') : "";
    return `"${v}"`;
  }).join(","));
...
/* =========================================================
   ADMIN: TICKET UPLOAD (stores under /public/tickets)
========================================================= */
const storage = multer.diskStorage({
  destination: (_req, _file, cb) => cb(null, "public/tickets"),
...
/* =========================================================
   BANK DETAILS (FROM HEROKU CONFIG VARS)
========================================================= */
app.get("/account.json", (_req, res) => {
  res.json({
